...
(function () {
 ...
 $(document).on("pagebeforeshow", "#page_field", function(event){
   ...
   $.getJSON('/api/fields/'+ fieldID +'/datastreams', null, function (datastreams) {
 ...
 // ------------------
  ...
  $(document).on("pagebeforeshow", "#page_alert", function(event) {
    ...
    function markup(alrt) {
	     //-センサ１
	     var select1 = $("#sensor_choice1");
	     select1[0].selectedValue = alrt.sid;
		   select1.selectmenu("refresh");            
	     //-大小
	     var select2 = $("#rel-choice1");
	     select2[0].selectedIndex = alrt.relation;
		   select2.selectmenu("refresh");            
	     // 閾値
	     document.querySelector('#threshold1').innerHTML=alrt.threshold;
    }
    
    var aid = $('#alert').attr('aid');    
    if (! isNaN(aid)) {    
	    $.getJSON('/api/alerts/'+ aid , null, function (alrt) {
	      // 取得したアラート情報を表示
	      $('#aid').val(aid);
	      ...
	      select2[0].selectedIndex = 2;
		    select2.selectmenu("refresh");            
	      // 閾値
	      document.querySelector('#threshold1').innerHTML=alrt.threshold1;
	    });
    }
  });   
